  ...
  server.on("request",function(request,response){
    ...
    fs.readFile(fullpath, (err, data) => {
      ...
      if (err.code == "ENOENT") { //404 Not Found
        ...
        if (fullpath.endsWith('.html')) require("fs").promises.readFile(fullpath.replace(/\.html$/,'.md')).then(function(dx){response.end(amistad(dx))},function(er){response.end(nsw_errors.handle(err,fullpath))});
